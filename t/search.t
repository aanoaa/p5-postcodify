...
subtest '도로명주소로 검색하는 경우' => sub {
    my $rs  = $p->search('서울시 종로구 세종대로 175');
    my $sql = as_query($rs);
    like $sql, qr/keyword_crc32/;
    like $sql, qr/`me`.`num_major` =/;

    $rs  = $p->search('175, sejongdaero, Jongro-gu, Seoul');
    $sql = as_query($rs);
