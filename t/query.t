...
unlike( $q->parse('서울,광진구'), qr/,/, '검색어에서 불필요한 문자를 제거한다' );
like( $q->parse('00번지 0호'), qr/00-0/, '지번을 00번지 0호로 쓴 경우 검색 가능한 형태로 변환한다' );
like( $q->parse('화양 3동'), qr/화양3동/, '행정동, 도로명 등의 숫자 앞에 공백에 있는 경우 붙여쓴다' );
is( $q->parse('Achasan-ro, Gwangjin-gu, Seoul')->lang, 'EN', '영문 도로명주소 또는 지번주소인지 확인한다' );
is( $q->parse('Nonsan P.O.Box, Nonsan-si, Chungcheongnam-do, Korea')->sort, 'POBOX', '영문 사서함 주소인지 확인한다' );
